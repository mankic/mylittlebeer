...
def sign_up_view(request):
    if request.method == 'GET':
        ...
        if password != password2:
        ...
        elif len('email') > 30 or len('password') > 18:
            return render(request, 'user/signup.html', {'error': '줄여주세요!'})
        elif '@' not in email or '.' not in email:
            return render(request, 'user/signup.html', {'error': '이메일 형식을 확인해주세요!'})
        else:
            ...
            exist_password = get_user_model().objects.filter(password=password)
            ...
            if exist_user:
                ...
                return render(request, 'user/signup.html',
                              {'error': '패스워드가 존재합니다'})  # 패스워드가 존재하기 때문에 사용자를 저장하지 않고 회원가입 페이지를 다시 띄움
            else:
